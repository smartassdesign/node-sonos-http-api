...
function HttpAPI(discovery, settings) {
  ...
  const runWorkflowAction = require('./actions/runworkflow');
  runWorkflowAction(this);
  const customPlayPause = require('./actions/custom_playpause');
  customPlayPause(this);
  ...
  this.requestHandler = function (req, res) {
    ...
    function sendResponse(code, body) {
      ...
      res.write(Buffer.from(jsonResponse));  // Updated to use Buffer.from
      res.end();
    }
